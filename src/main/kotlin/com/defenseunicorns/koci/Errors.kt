 ...
 *
 * This class models the error response format defined in the OCI spec,
 * which consists of an array of errors. Each registry API error response
 * must include at least one error.
 *
 * @property errors List of actionable errors returned by the registry
 * @property status HTTP status code of the response
 * @see <a href="https://github.com/opencontainers/distribution-spec/blob/main/spec.md#error-codes">OCI Spec: Errors</a>
 */
@Serializable
 ...
 *
 * These codes identify the error type that occurred on the registry.
 * Error codes must be uppercase, using only alphanumeric characters and underscores.
 *
 * @see <a href="https://github.com/opencontainers/distribution-spec/blob/main/spec.md#error-codes">OCI Spec: Error Codes</a>
 */
 ...
 *
 * Each error contains a code identifying the error type, a message for the client,
 * and optional detail information specific to the error type.
 *
 ...
 *
 * This sealed class provides a hierarchy of exceptions for different types of
 * errors that can occur when interacting with OCI registries, including content
 * verification failures, API errors, and authentication issues.
 */
 ...
 *
 * This function handles error responses from OCI registries by parsing the
 * JSON error response and throwing an appropriate exception. It is used to
 * convert HTTP errors into typed exceptions for better error handling.
 *
