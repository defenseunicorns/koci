...
class RegistryTest {
  ...
  private val storage = runBlocking { Layout.create(tmp.toString()) }.getOrThrow()
  ...
  fun resolve() = runTest {
    assertEquals(
      INDEX_MEDIA_TYPE,
      registry.repo("dos-games").resolve("1.1.0").getOrThrow().mediaType,
    )
    assertEquals(
      MANIFEST_MEDIA_TYPE,
      registry
  ...
  fun `fetch a layer`() = runTest {
    ...
    val manifest =
      repo
        .resolve("1.1.0")
        .map { desc ->
          repo.index(desc).map { repo.manifest(it.manifests.first()).getOrThrow() }.getOrThrow()
        }
        .getOrThrow()
    ...
    assertEquals(true, storage.exists(manifest.config).getOrThrow())

    assertEquals(true, storage.remove(manifest.config).getOrThrow())
  ...
  fun `fetch a layer, cancelling multiple times`() = runTest {
    ...
    assertEquals(true, storage.exists(layer).getOrThrow())

    assertEquals(true, storage.remove(layer).getOrThrow())
  ...
  fun `pull and remove dos-games`() = runTest {
    ...
    for (layer in arm64Manifest.layers) {
      assertFalse { storage.exists(layer).getOrThrow() }
    }
    assertFalse { storage.exists(arm64Manifest.config).getOrThrow() }
  }
  ...
  fun `concurrent pulls + removals`() {
    ...
    assertDoesNotThrow {
      runTest(timeout = kotlin.time.Duration.parse("PT30S")) {
        ...
        val r1 = async { storage.remove(descriptor).getOrThrow() }
        val r2 = async { storage.remove(descriptor).getOrThrow() }
        awaitAll(r1, r2)
  ...
  fun `upload a layer and tag an artifact`() = runTest {
    ...
    for (at in cancelAt) {
      launch {
          repo
            ...
            .onCompletion { e ->
              assertIs<CancellationException>(e)
              assertTrue { repo.exists(tmp15Desc).isFailure }
            }
            .collect { progress ->
    ...
    repo.tag(manifest, "latest").also { res ->
      ...
      assertTrue { repo.remove(manifestDesc).getOrThrow() }
    }
  }
  ...
  fun `mounting blobs from another repository`() = runTest {
    ...
    assertFalse(
      targetRepo.exists(existingBlob).getOrDefault(false),
    ...
    assertTrue(
      targetRepo.exists(existingBlob).getOrThrow(),
